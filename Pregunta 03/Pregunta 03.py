...
cursor.execute('''CREATE TABLE entradas(codigo_entrada text, cine text, pelicula_elegida text, funcion_elegida text, cantidad_entradas integer)''')
...
def main():
    ...
    while not terminado:
		...
		if opcion == '1':
			...
			cursor.execute('''INSERT INTO entradas(codigo_entrada, cine, pelicula_elegida, funcion_elegida, cantidad_entradas)''')
